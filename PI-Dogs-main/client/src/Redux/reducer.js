import {
  GET_ALL_DOGS,
  GET_ALL_DOGS_BY_NAME,
  GET_ALL_DOGS_BY_TEMPERAMENT,
  GET_ALL_DOGS_EXISTENT_AND_CREATED,
  GET_ALL_TEMPERAMENTS,
  GET_DETAIL_DOGS,
  GET_ORDER_BY_DOGS,
  GET_ORDER_BY_PESO_DOGS,
  POST_DOG,
} from "./actions";
...
const rootReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case GET_ALL_DOGS_EXISTENT_AND_CREATED:
      const nuevo = state.all_dogs; //me trae todo
      const dogsexiste = state.dogsexiste; //me trae todo pero se modifica siempre
      const a = [];
      const b = [];
    ...
    case GET_ORDER_BY_PESO_DOGS:
      if (action.payload === "ascPeso") {
        const ascPeso = state.dogs.sort((a, b) => a.weight[0] - b.weight[0]);
        return {
          ...state,
          dogs: ascPeso,
        };
      } else if (action.payload === "descPeso") {
        const descPeso = state.dogs.sort((a, b) => b.weight[0] - a.weight[0]);
        return {
          ...state,
          dogs: descPeso,
        };
